...
public class AlarmReminderProvider extends ContentProvider {
    ...
    private Uri insertReminder(Uri uri,ContentValues contentValues){
        ...
        long id=database.insert(AlarmReminderContract.AlarmReminderEntry.TABLE_NAME,null,contentValues);
        if(id== -1)
        {
    ...
    private int updateReminder(@NonNull Uri uri, @Nullable ContentValues values, @Nullable String selection, @Nullable String[] selectionArgs)
    {
        if (values.size()==0){
            return 0;
        }
